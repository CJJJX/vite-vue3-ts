<script setup lang="ts">
    ...
    const talentId = router.currentRoute.value.params.resumeId
    ...
    const sendSubmit = async () => {
      state.loading = true
      const res:any = await chatMessageContentAdd({
    ...
    const getChatMessageContent = async () => {
      const res:any = await chatMessageContent({
        receive_id: receiveId,
        things_id: talentId,
        things_type: 1,
      })
      if(res){
        state.list = res.data
        state.talentName = res.data && res.data[0].receive_name || '人才'
      }else{
        showToast(res.msg)
      }
    }
    ...
    onBeforeUnmount(()=>{stopSetInterval()})
    createInterval()
...
<template>
    <van-nav-bar :title="state.talentName" left-arrow @click-left="leftBack"/>
    <div class="talk-page">
        <dl>
