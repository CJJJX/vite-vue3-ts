...
<template>
     ...
     <div class="person-top">
        <van-icon name="arrow-left" size="1.3rem" @click="leftBack"/>
        <h3>IT企业人才</h3>
        <h4>一站式驻场/远程灵活用工</h4>
     </div>
     ...
     <div class="person-form">
        ...
        <van-form @submit="submit" class="form">
            ...
            <van-field v-model="state.sex" @click="state.showSex=true" readonly label="性别:" placeholder="请输入您的性别"/>
            <van-action-sheet v-model:show="state.showSex" :actions="sexList"
             cancel-text="取消" close-on-click-action @select="setSelect"/>
            <van-field v-model="state.phone" label="电话:" placeholder="请输入您的电话" type="number"/>
            <van-field v-model="state.age" label="年龄:" placeholder="请输入您的真实年龄" type="number"/>
            <van-field v-model="state.address" label="地址:" placeholder="请输入您的现住址"/>
            <van-field v-model="state.work_data" label="经历:" placeholder="请输入您的工作年限" type="number"/>
            <van-field v-model="state.introduce" label="介绍:" placeholder="请简单填写个人介绍和工作内容"
